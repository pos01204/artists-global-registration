  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-gradient-to-r from-idus-orange-light/40 to-amber-50 
                      rounded-2xl p-5 border border-idus-orange/20">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-xl bg-idus-orange flex items-center justify-center shadow-lg shadow-idus-orange/20">
            <Sparkles className="w-6 h-6 text-white" />
          </div>
          <div>
            <div className="font-bold text-idus-black text-lg">idus 공식 번역 프롬프트</div>
      ...
      <div className="bg-idus-gray rounded-xl p-4">
        <div className="text-sm font-semibold text-idus-black mb-2 flex items-center gap-2">
          <span className="w-5 h-5 rounded-full bg-idus-orange text-white text-xs flex items-center justify-center">?</span>
          사용 방법
        </div>
        <ol className="text-sm text-idus-black-70 space-y-1.5 ml-7">
        ...
        {PROMPTS.map((item) => (
          ...
          >
            ...
            <button
              ...
              className="w-full flex items-center justify-between p-4 hover:bg-idus-gray/50 transition-colors"
            >
              <div className="flex items-center gap-3">
                <span className="text-3xl">{item.flag}</span>
                <div className="text-left">
                  <div className="font-semibold text-idus-black">{item.title}</div>
                  <div className="text-xs text-idus-black-50">{item.description}</div>
                </div>
              </div>
              <motion.div
              ...
              >
                <ChevronDown className="w-5 h-5 text-idus-black-30" />
              </motion.div>
            </button>
              ...
              {expandedId === item.id && (
                ...
                >
                  <div className="px-4 pb-4">
                    {/* 프롬프트 박스 */}
                    <div className="relative bg-gradient-to-br from-idus-gray to-slate-100 rounded-xl p-4 border border-idus-black-10">
                      <pre className="text-sm text-idus-black whitespace-pre-wrap font-mono leading-relaxed pr-16">
                        {item.prompt}
                      </pre>
                      ...
                      <motion.button
                        ...
                        className={`
                          ...
                          px-3 py-2 rounded-lg border transition-all text-sm font-medium shadow-sm
                          ${copiedId === item.id 
                    ...
                    {/* 입력 위치 안내 */}
                    <div className="mt-3 flex items-center gap-2 text-xs text-idus-black-50 bg-amber-50 px-3 py-2 rounded-lg border border-amber-200">
                      <span className="text-amber-500">💡</span>
                      <span className="text-amber-700">[여기에 한글 상세설명을 입력] 부분에 번역할 내용을 넣어주세요</span>
                    </div>
                  </div>
                </motion.div>
              )}
      ...
      <motion.a
        ...
        className="flex items-center justify-center gap-2 p-4 
                   ...
                   rounded-xl text-white font-bold shadow-lg shadow-emerald-500/20
      ...
      <a
        ...
        className="flex items-center justify-center gap-2 p-3 
                   ...
                   hover:bg-idus-black-10 transition-all text-sm"
      >
        <span>📖</span>
